...
BEGIN
    ...
    INSERT INTO test.dim_schedule_version (version_id, valid_from, valid_to)
    ...
    WITH staging_to_fct AS (
        ...
        FROM test.staging_event_session e
        LEFT JOIN test.dim_venues v ON e.venue_name = v.venue_name
			AND v.is_current = TRUE
        LEFT JOIN test.dim_sports s ON e.sport_discipline = s.sport_discipline
			AND s.is_current = TRUE
		LEFT JOIN test.dim_subvenues sv on e.subvenue_name = sv.subvenue_name
			AND v.venue_id = sv.venue_id
			AND sv.is_current = TRUE
		LEFT JOIN test.dim_regions r on e.region_name = r.region_name
			AND r.is_current = TRUE
		LEFT JOIN test.dim_zones z on e.zone_name = z.zone_name
			AND z.is_current = TRUE
		LEFT JOIN test.dim_clusters cl on e.cluster_name = cl.cluster_name
			AND cl.is_current = TRUE
		LEFT JOIN test.dim_calendar c ON e.event_day = c.event_day
    ...
    changed_events AS (
        SELECT 
            md5(
                COALESCE(s.venue_id::TEXT, 'NULL') || '-' ||
                COALESCE(s.sport_id::TEXT, 'NULL') || '-' ||
                COALESCE(s.day_id::TEXT, 'NULL') || '-' ||
                s.session_id::TEXT || '-' ||
        ...
        LEFT JOIN test.fct_event_session e ON 
            md5(
                COALESCE(s.venue_id::TEXT, 'NULL') || '-' ||
                COALESCE(s.sport_id::TEXT, 'NULL') || '-' ||
                COALESCE(s.day_id::TEXT, 'NULL') || '-' ||
    ...
    , inserted_events AS (
        INSERT INTO test.fct_event_session (
            hash_key, schedule_version_id, current_version_id, version_array, sport_id, venue_id, 
			subvenue_id, region_id, zone_id, cluster_id, day_id, session_id, 
        ...
        SELECT 
            ...
            END,
            sport_id, venue_id, subvenue_id, region_id, zone_id, cluster_id, day_id, session_id,
            date_start, start_time, date_end, end_time, competition_type, event_type, 
            gross_seats, seat_kill, est_pct_ticksold, net_seats, est_sold_seats, workforce, unticketed,
    ...
    , update_old_records AS (
        UPDATE test.fct_event_session f
...
SELECT * FROM get_changes_in_date_range('2023-11-30'::DATE, '2024-01-06'::DATE);